...
export default function PendingRequestsScreen({ navigation }) {
  ...
  const handleAccept = async (id, username) => {
    ...
    if (response.ok) {
      ...
      try {
        const messageRespnse = await fetch('http://localhost:3000/message/send', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          ...
          body: JSON.stringify({
            ...
            data: null,
            message: `${loggedInUser} accepted your friend request now you can invite each other to join tournaments!`
          })
        });
        const messageData = await messageRespnse.json();
        if (messageRespnse.ok) {
          console.log('Message sent successfully', messageData);
        } else {
          console.log('Sending message failed', messageData.error);
        }
      }
      catch (err) {
        console.error(err);
